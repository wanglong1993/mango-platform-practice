...
import cn.siques.mangocore.entity.SysRoleMenuExample;
...
public class SysRoleMenuSqlProvider {
    ...
    protected void applyWhere(SysRoleMenuExample example, boolean includeExamplePhrase) {
        ...
        List<SysRoleMenuExample.Criteria> oredCriteria = example.getOredCriteria();
        boolean firstCriteria = true;
        for (int i = 0; i < oredCriteria.size(); i++) {
            SysRoleMenuExample.Criteria criteria = oredCriteria.get(i);
            if (criteria.isValid()) {
                if (firstCriteria) {
                    firstCriteria = false;
                } else {
                ...
                sb.append('(');
                List<SysRoleMenuExample.Criterion> criterions = criteria.getAllCriteria();
                boolean firstCriterion = true;
                for (int j = 0; j < criterions.size(); j++) {
                    SysRoleMenuExample.Criterion criterion = criterions.get(j);
                    if (firstCriterion) {
