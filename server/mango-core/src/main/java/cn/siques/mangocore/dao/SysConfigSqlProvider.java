...
import cn.siques.mangocore.entity.SysConfigExample;
import cn.siques.mangocore.entity.SysConfigWithBLOBs;
...
public class SysConfigSqlProvider {
    ...
    protected void applyWhere(SysConfigExample example, boolean includeExamplePhrase) {
        ...
        List<SysConfigExample.Criteria> oredCriteria = example.getOredCriteria();
        boolean firstCriteria = true;
        for (int i = 0; i < oredCriteria.size(); i++) {
            SysConfigExample.Criteria criteria = oredCriteria.get(i);
            if (criteria.isValid()) {
                if (firstCriteria) {
                    firstCriteria = false;
                } else {
                ...
                sb.append('(');
                List<SysConfigExample.Criterion> criterions = criteria.getAllCriteria();
                boolean firstCriterion = true;
                for (int j = 0; j < criterions.size(); j++) {
                    SysConfigExample.Criterion criterion = criterions.get(j);
                    if (firstCriterion) {
